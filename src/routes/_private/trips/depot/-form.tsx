...
export default function TripForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    if(!id && user){
      const routes = user?.activeOrganization?.metadata?.routeConfigs?.[user?.activeOrganization?.metadata?.routeConfigs?.length - 1]?.routes || []
      const price = user?.activeOrganization?.metadata?.fuelPrice || 0
      if(tripRoutesDepot !== routes){
        setTripRoutesDepot(routes)
      }
      if(fuelPrice !== price){
